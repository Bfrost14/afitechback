...
export class ListeCoursComponent implements OnInit, AfterViewInit {
    ...
    expandedElement: NewCours | null;
    selectCoursEdit: NewCours
    selectedColumn = [
        { key: 'matricule', value: '' },
        { key: 'email', value: '' },
        { key: 'filiere', value: '' },
        ...
        merge(this.sort.sortChange, this.paginator.page)
            .pipe(
                ...
                switchMap(() => {
                    ...
                    return this._coursService!.query(
                        {page: this.paginator.pageIndex, 
                            size: this.paginator.pageSize, 
                            sort: this.sort.active+","+this.sort.direction}
    ...
    fiche: boolean = false;
    setDataSource(cours: NewCours) {
        console.log('>>>>>>>>>>>>>>>.. data set >>>>>>>>>>>>>>>>>>', cours);
        if (this.fiche) {
            this.coursId = null
            this.fiche = false;
    ...
    setDataSourceEdit(cours: NewCours) {
        console.log('>>>>>>>>>>>>>>>.. data set >>>>>>>>>>>>>>>>>>', cours);
        if (this.add) {
            this.coursIdEdit = null
            this.selectCoursEdit = null
            this.add = false;
            this.data = [...this.dataSource];
        } else {
            this.coursIdEdit = cours.id
            this.selectCoursEdit = cours
            this.add = true;
            this.data = [cours];
        }
    }
