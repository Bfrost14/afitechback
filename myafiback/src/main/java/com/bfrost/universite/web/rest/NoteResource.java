...
/**
 * REST controller for managing {@link com.bfrost.universite.domain.Note}.
 */
@RestController
...
public class NoteResource {
    ...
    public NoteResource(NoteService noteService, NoteRepository noteRepository, PaginationService paginationService) {
    ...
    public ResponseEntity<List<NoteDTO>> createNote(@RequestBody List<NoteDTO> noteDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/notes/" + noteDTO.get(0).getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, noteDTO.get(0).getId().toString()))
            .body(noteDTO);
    }
