...
/**
 * REST controller for managing {@link com.bfrost.universite.domain.Matiere}.
 */
@RestController
...
public class MatiereResource {
    ...
    public MatiereResource(MatiereService matiereService, MatiereRepository matiereRepository, PaginationService paginationService) {
    ...
    public ResponseEntity<List<MatiereDTO>> createMatiere(@Valid @RequestBody List<MatiereDTO> matiereDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/matieres/" + matiereDTO.get(0).getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, matiereDTO.get(0).getId().toString()))
            .body(matiereDTO);
    }
